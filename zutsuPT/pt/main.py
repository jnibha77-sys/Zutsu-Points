...
# --- Coordinates ---
# Each team row has (x, y) positions for each field including the logo.
# There are 12 teams total. Coordinates to be set per user's specifications.
# Format: { "logo": (x, y), "rank": (x, y), "name": (x, y), "mp": (x, y),
#            "booyah": (x, y), "elims": (x, y), "place": (x, y), "total": (x, y) }

# --- Team Position Coordinates ---
# Each entry defines bounding boxes and positions for one team row.
#
# KEY FIELDS:
#   logo       = (x, y) fallback top-left paste position for logo
#   logo_size  = pixel size to resize logo (width & height)
#   logo_box   = (x1, y1, x2, y2) bounding box — logo is centred inside it
#   name       = (x, y) fallback top-left position for team name
#   name_box   = (x1, y1, x2, y2) bounding box — name is centred (or left-aligned) inside it
#   name_align = "left" to left-align name inside name_box; omit for centre
#   name_fs    = font size for team name
#   num_fs     = font size for stat numbers
#   *_box      = (x1, y1, x2, y2) bounding box for mp/booyah/elims/place/total — text centred inside
#
# LEFT SIDE: Teams 1–5  |  RIGHT SIDE: Teams 6–12
# Teams 2–5: each row is +34px on the y-axis from the previous row (starting at y=300)

# Shared sizing for Teams 2–12 (change these to adjust all rows at once)
ROW_LOGO_SIZE = 110   # logo width & height in pixels
ROW_NAME_FS = 85     # font size for team name
ROW_NUM_FS = 85      # font size for stat numbers (mp, booyah, elims, place, total)

team_positions = [

    # ── Team 1 — large featured card (top-left) ──
    # logo_box: centred inside (97,208)→(187,256), logo_size controls actual logo dimensions
    # name_box: centred inside (235,200)→(415,230)
    # stat boxes: centred, y range 246→266
    ...
    # All use bounding boxes. Logo & stats centred inside boxes; name LEFT-aligned.
    # Each row shifts +230px on the y-axis from the previous row.
    # logo_box: (465,y)→(613,y+146) | name_box: (614,y)→(1464,y+146) LEFT-aligned
    # mp_box: (1465→1714) | booyah_box: (1716→2000) | elims_box: (2001→2220)
    # place_box: (2221→2438) | total_box: (2590→2765)
    ...
    # All use bounding boxes. Logo & stats centred inside boxes; name LEFT-aligned.
    # Each row shifts +225px on the y-axis from the previous row.
    # logo_box: (3221,y)→(3361,y+139) | name_box: (3362,y)→(4158,y+139) LEFT-aligned
    # mp_box: (4159→4408) | booyah_box: (4409→4686) | elims_box: (4687→4911)
    # place_box: (4912→5138) | total_box: (5302→5480)
...
def draw_team(img, draw, pos, rank, team, text_color="black"):
    ...
    # If "bold" flag is set, use bold font for ALL stat numbers; otherwise only for total
    all_bold = pos.get("bold", False)
    nf_num = get_font_bold(pos.get("num_fs", FONT_SIZE)) if all_bold else get_font(pos.get("num_fs", FONT_SIZE))         # numbers font
